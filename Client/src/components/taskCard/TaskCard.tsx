...
const TaskCard: React.FC<IProps> = ({ task, rootStyles }) => {
  return (
    <Link to={`${TASK_ROUTE}?id=${task.id}`} key={task.id} className={rootStyles?.TaskListElement || 'TaskListElement'}>
      ...
      <div className={rootStyles?.TaskListElement__fiveStars || 'TaskListElement__fiveStars'}>
        <FiveStars showRatingValue={false} active={false} incomingRatingValue={task.rating} />
